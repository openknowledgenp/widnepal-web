...
import { BLOGS_WITH_CATEGORY, BLOGS_ERROR_MESSAGES } from '../../../graphql/blog.queries';
...
const Blog = () => {
  ...
  // Create a query hook
  const { data, loading, error } = useQuery(BLOGS_WITH_CATEGORY(slug));
  ...
  return (
    <PageLayout title={`Blogs | Category: ${slug}`}>
      {blogError
        ?
